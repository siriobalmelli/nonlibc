...
int test_nm_div_ceil()
{
	...
	/* divides evenly */
	NB_err_if(nm_div_ceil(42, 7) != 6, "%"PRIu64, nm_div_ceil(42, 7));
	/* divides unevenly: expect division result +1 */
	NB_err_if(nm_div_ceil(10, 6) != 2, "%"PRIu64, nm_div_ceil(10, 6));
	/* divides into 0: expect 1 */
	NB_err_if(nm_div_ceil(5, 9) != 1, "%"PRIu64, nm_div_ceil(5, 9));
...
int test_nm_next_pow2()
{
	...
	*/
	NB_err_if(nm_next_pow2_32(0) != 0, "%"PRIu32, nm_next_pow2_32(0));
	NB_err_if(nm_next_pow2_32(1) != 1, "%"PRIu32, nm_next_pow2_32(1));
	NB_err_if(nm_next_pow2_32(64) != 64, "%"PRIu32, nm_next_pow2_32(64));
	...
	*/
	...
	NB_err_if(nm_next_pow2_64(1) != 1, "%"PRIu64, nm_next_pow2_64(1));
	NB_err_if(nm_next_pow2_64(64) != 64, "%"PRIu64, nm_next_pow2_64(64));
	NB_err_if(nm_next_pow2_64(33) != 64, "%"PRIu64, nm_next_pow2_64(33));
	NB_err_if(nm_next_pow2_64(63) != 64, "%"PRIu64, nm_next_pow2_64(63));

	/* check very large numbers */
	NB_err_if(nm_next_pow2_64(UINT64_MAX >> 1) != 0x8000000000000000,
			"%"PRIu64, nm_next_pow2_64(UINT64_MAX >> 1));
	NB_err_if(nm_next_pow2_64(UINT64_MAX) != 0, "%"PRIu64, nm_next_pow2_64(UINT64_MAX));
...
int test_nm_bit_pos()
{
	...
	NB_err_if(nm_bit_pos(0x0) != 0, "%"PRIuFAST8, nm_bit_pos(0x0));
	/* first bit set for both instances */
	NB_err_if(nm_bit_pos(0x1) != 1, "%"PRIuFAST8, nm_bit_pos(0x1));
	NB_err_if(nm_bit_pos(0x3) != 1, "%"PRIuFAST8, nm_bit_pos(0x3));
	/* MSb set */
	NB_err_if(nm_bit_pos(0x8000000000000000) != 64,
	...
	NB_err_if(strcmp("en_one", prints[nm_bit_pos(en_one)]), "");
	...
	*/
	...
	NB_err_if((size_t)1 << shift != pow2,
		"%zu != %zu", (size_t)1 << shift, pow2);
	/* shift factor to multiply by pow2 */
	NB_err_if((size_t)42 << shift != (size_t)42 * pow2,
