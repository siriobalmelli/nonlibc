...
/**
 * Varargs function: test the use of the NB_(die|err|wrn)_on()
 * macros when reproducing a call to this function.
 */
const char *varargs(const char *message, ...)
{
	static char buf[256];
	va_list	    arglist;
...
int basic_function()
{
	 ...
	 */
	NB_err_if(1 != 1, "this will never error; err_cnt is %d", err_cnt);
...
struct malloc_err_example *malloc_error()
{
	...
	NB_die_if(!(ret = malloc(sizeof(struct malloc_err_example))),
	 ...
	 */
	NB_die_if(!(ret->ptr_a = malloc(sizeof(uint64_t))), "could not malloc() %zu bytes",
		  sizeof(uint64_t));
...
#pragma GCC diagnostic ignored "-Woverflow"
	...
	NB_die_if(!(ret->ptr_b = malloc(insane)), "could not malloc() %zu (insane amount) bytes",
		  insane);
...
int main()
{
	 ...
	 */
	NB_die_if((res = basic_function()) != 2, "sum ting willy wong heah");
	...
	NB_wrn_on(2 == 2);
	NB_wrn_on(!!varargs("hello %d", 42)); /* always print */

	/**
	 * Show error handling for a function which allocates
	 * (potentially multiple) memory regions.
	 */
	NB_die_if((a_pointer = malloc_error()) != NULL, "we expected this to fail");

	/* malloc (with safety check); then print a buffer */
	const size_t len = 125;
	NB_die_if(!(a_malloc = malloc(len)), "len = %zu", len);
	NB_dump(a_malloc, len, "printing buffer len %zu", len);
